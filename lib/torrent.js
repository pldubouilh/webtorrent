...
function Torrent (torrentId, client, opts) {
  ...
  this.initialFileSelection = opts.fileSelection
...
Torrent.prototype._onMetadata = function (metadata) {
  ...
  // start off selecting file selection if specified, else the entire torrent with low priority
  if (self.initialFileSelection) {
    self.setFileSelection(self.initialFileSelection)
  } else if (self.pieces.length !== 0) {
    self._selectPieces(0, self.pieces.length - 1, false)
  }
...
Torrent.prototype.setFileSelection = function (filesSelected) {
  var self = this
  if (self.destroyed) throw new Error('torrent is destroyed')

  self._selections = []

  self.files = self.files.map(function (f, i) {
    f.selected = false
    return f
  })

  for (var i = 0; i < filesSelected.length; i++) {
    var j = filesSelected[i]

    if (!self.files[j]) {
      throw new Error('file selected non-existing')
    }

    self.files[j].selected = true
    self._addToSelection(self.files[j]._startPiece, self.files[j]._endPiece)
  }
...
Torrent.prototype._addToSelection = function (start, end, priority, notify) {
  var self = this
  if (self.destroyed) throw new Error('torrent is destroyed')
...
Torrent.prototype._selectPieces = function (start, end, priority, notify) {
  var self = this
  if (self.destroyed) throw new Error('torrent is destroyed')

  self._addToSelection(start, end, priority, notify)
...
Torrent.prototype._deselectPieces = function (start, end, priority) {
  var self = this
  if (self.destroyed) throw new Error('torrent is destroyed')
